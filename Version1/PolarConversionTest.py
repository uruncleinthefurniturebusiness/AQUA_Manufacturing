...
def plot_gcode_movements(movements, title="G-code Movements", random_colors=False, color_by_speed=False):
    ...
    cmap = plt.cm.get_cmap('coolwarm')  # Colormap for speed visualization
    ...
    for move in movements:
        ...
        if (first_line):
            ax.plot([theta_start, theta_end], [r_start, r_end], color='r',
                    label=f"Move from ({x_start}, {y_start}) to ({x_end}, {y_end})")
            first_line = False
        else:
            ax.plot([theta_start, theta_end], [r_start, r_end], color=color,
                    label=f"Move from ({x_start}, {y_start}) to ({x_end}, {y_end})")
...
def do_plotting(original_movements, modified_movements):
    ...
    plot_gcode_movements(modified_movements, title="Modified G-code Movements (Speed Color Temperature)",
                         color_by_speed=True)
...
def rotate_about_center(output, modified_movements):
    ...
    for i in range(num_angles - 1):
        modified_movements.append(((round(points[i][0], 3), round(points[i][1], 3)),
                                   (round(points[i + 1][0], 3), round(points[i + 1][1], 3)), rotation_speed))
...
def process_gcode_and_plot(input_file):
    ...
    with open(input_file + '_processed.gcode', 'w') as output:
        ...
        while i < len(lines):
            ...
            while ((not lines[i].startswith("G1") and not lines[i].startswith("G0")) or 'X' not in lines[
                i] or 'Y' not in lines[i]):
                # print(lines[i])
            ...
            if split_moves:
                ...
                if (distance_to_singularity(x, y) > SPLIT_START_RADIUS and distance_to_singularity(next_x,
                                                                                                   next_y) > SPLIT_START_RADIUS):
                    # goes through circle
                    ...
                    modified_movements.append(
                        ((x, y), (intersection_points[0][0], intersection_points[0][1]), new_speed))
                    # print(modified_movements[-1])
                    if next_e != None:
                        fractional_extruded = fraction_extruded(next_e - prev_e, line_length(x, y, next_x, next_y),
                                                                line_length(x, y, intersection_points[0][0],
                    ...
                    if point_line_distance(intersection_points[0], intersection_points[1]) <= CENTER_R:  # split it
                        ...
                        modified_movements.append(
                            ((intersection_points[0][0], intersection_points[0][1]), (x_new_end, y_new_end), MIN_SPEED))

                        # get fraction extruded just through whole circle
                        fraction_extruded_center_circle = fraction_extruded(next_e - prev_e,
                                                                            line_length(x, y, next_x, next_y),
                                                                            line_length(intersection_points[1][0],
                        ...
                        if next_e != None:
                            ...
                            fractional_extruded += fraction_extruded(fraction_extruded_center_circle,
                                                                     line_length(intersection_points[1][0],
                                                                                 intersection_points[1][1],
                                                                                 intersection_points[0][0],
                                                                                 intersection_points[0][1]),
                                                                     line_length(intersection_points[0][0],
                                                                                 intersection_points[0][1], x_new_end,
                                                                                 y_new_end))
                            write_new_gcode(parts, modified_movements, output, round(prev_e + fractional_extruded, 5))
                        else:
                            write_new_gcode(parts, modified_movements, output)
                    ...
                    if next_e != None:
                        fractional_extruded += fraction_extruded(next_e - prev_e, line_length(x, y, next_x, next_y),
                                                                 line_length(intersection_points[1][0],
                                                                             intersection_points[1][1], next_x, next_y))
                        write_new_gcode(parts, modified_movements, output, round(prev_e + fractional_extruded, 5))
                    else:
                        write_new_gcode(parts, modified_movements, output)
                ...
                ##########################
                    ...
                    if point_line_distance(intersection_points[0], (next_x, next_y)) <= CENTER_R:  # split it
                        ...
                        if next_e != None:
                            ...
                            fractional_extruded += fraction_extruded(fraction_extruded_center_circle,
                                                                     line_length(intersection_points[0][0],
                                                                                 intersection_points[0][1], next_x,
                                                                                 next_y),
                                                                     line_length(intersection_points[0][0],
                                                                                 intersection_points[0][1], x_new_end,
                                                                                 y_new_end))
                        ...
                        if next_e != None:
                            ...
                            fractional_extruded += fraction_extruded(fraction_extruded_center_circle,
                                                                     line_length(intersection_points[0][0],
                                                                                 intersection_points[0][1], next_x,
                                                                                 next_y),
                                                                     line_length(next_x, next_y, new_start[0],
                                                                                 new_start[1]))
                            write_new_gcode(parts, modified_movements, output, round(prev_e + fractional_extruded, 5))
                        else:
                            write_new_gcode(parts, modified_movements, output)
                ...
                ########################
                    ...
                    if point_line_distance(intersection_points[0], (x, y)) <= CENTER_R:  # split it
                        ...
                        fraction_extruded_center_circle = fraction_extruded(next_e - prev_e,
                                                                            line_length(x, y, next_x, next_y),
                                                                            line_length(intersection_points[0][0],
                                                                                        intersection_points[0][1], x,
                                                                                        y))
                        if next_e != None:
                            # use the amount extruded through whole circle to find just the amount extruded to (or away) the center
                            fractional_extruded = fraction_extruded(fraction_extruded_center_circle,
                                                                    line_length(intersection_points[0][0],
                                                                                intersection_points[0][1], x, y),
                                                                    line_length(x, y, x_new_end, y_new_end))
                            write_new_gcode(parts, modified_movements, output, round(prev_e + fractional_extruded, 5))
                        else:
                            write_new_gcode(parts, modified_movements, output)
                    ...
                    if next_e != None:
                        fractional_extruded += fraction_extruded(next_e - prev_e, line_length(x, y, next_x, next_y),
                                                                 line_length(intersection_points[0][0],
                                                                             intersection_points[0][1], next_x, next_y))
                        write_new_gcode(parts, modified_movements, output, round(prev_e + fractional_extruded, 5))
                    else:
                        write_new_gcode(parts, modified_movements, output)
                    ...
                    if point_line_distance((next_x, next_y), (x, y)) <= CENTER_R:  # split it
                        ...
                        if next_e != None:
                            ...
                            fractional_extruded = fraction_extruded(next_e - prev_e, line_length(next_x, next_y, x, y),
                                                                    line_length(x, y, 0, 0))
                            write_new_gcode(parts, modified_movements, output, round(prev_e + fractional_extruded, 5))
                        else:
                            write_new_gcode(parts, modified_movements, output)
...
##########call the function
input_gcode = 'Freaky_Cylinder'  # Replace with your actual G-code file
